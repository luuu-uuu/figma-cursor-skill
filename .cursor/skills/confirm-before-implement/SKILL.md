...
**原则**：若存在多种可选的实现方式，且**不同方式会实质影响交付结果**，须在确认方案时**明确将采用哪一种**，并得到用户同意；**不得**在未告知的情况下默认采用某一实现路径。

（示例：如「根据歌单生成推荐」，不同数据来源或策略往往得到**不同的曲目组成与整体气质**——须在对比表中摊开备选，让用户选定后再实现。）
...
2. **Proposed implementation** — **凡涉后端**：附 **分支/数据源一览**；默认优先采用 **对比表 + 短流程 + 参数表**（见上节）；无隐藏 fallback。
...
- [ ] Step 1b 待确认项已澄清。
- [ ] Step 2 已说明逻辑；**凡涉后端已列出全部路径与数据源并得到同意**（已尽量使用 **对比表 + 短流程 + 参数表**），未私自加备选逻辑。
- [ ] 仅在此之后：下载资源、改文件、跑命令。
